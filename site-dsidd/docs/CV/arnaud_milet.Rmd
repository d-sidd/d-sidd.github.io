...
- **Techniques**: Traitement et analyse de données, Datavisualisation/Cartographie, Développement de packages R, Recherche documentaire, Gestion de code avec Git, Formateur R, SQL, Metabase, Notion.
...
```{r list_reference, echo=FALSE,message=FALSE,warning=FALSE}
library(readxl)
library(tidyverse)
...
liste_references <- references_dsidd %>% 
  ...
  pivot_longer(cols = c(11:16),names_to = "Thématique" ) %>%  
  drop_na(value) %>% 
  mutate(`Thématique`=factor(`Thématique`)) %>% 
  group_by(across(-`Thématique`)) %>% 
  filter(as.numeric(Thématique)==min(as.numeric(`Thématique`))) %>% 
  ungroup() %>% 
